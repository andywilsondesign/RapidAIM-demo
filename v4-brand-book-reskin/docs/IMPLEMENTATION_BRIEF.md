...
1. Preserve the accessible badge model.
   The high, medium, low, offline, neutral, and entity badge variants remain the same semantic component model. Their contrast-oriented operational colors are intentionally separated from brand colors.

2. Keep Inter for digital UI.
   The Brand Book 2.0 Arial recommendation is treated as a print/system-template direction. Inter stays as the product UI typeface because it is already proven across desktop and mobile views and is in a similar neutral grotesque family.
...
7. Prefer right-angled product geometry.
   Brand Book 2.0 leans toward a more technical, right-angled visual language. Shared product radius tokens default to square corners, while `--radius-full` remains available for semantic controls such as avatars, status dots, sensor pins, and toggle thumbs.
...
V3 is the current brand-aligned handoff system:
...
Create a set of static RapidAIM pages and Storybook stories that all use the same component system and can be compared against V3.
